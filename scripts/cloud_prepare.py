...
def prepare(annotations_root, output, images_root=None, source_versions=None):
    source, output = Path(annotations_root).resolve(), Path(output).resolve()
    required = ["grading/grading_all.csv", "localize/disc_localization.csv"]
    if any(not (source / x).is_file() for x in required) or not (source / "folds").is_dir():
        raise ValueError("annotations_root must directly contain grading/, localize/, folds/")
    ...
    result = {"identity": identity, "images": images, "images_manifest_sha256": content_hash(images),
